                    ...
                    <div v-if="!showCoachCarousel && ((index === 0 && !isSignedIn && welcomeTypingComplete) || (isSignedIn && showFileUploadForReturningUser && isReadyToUploadMessage(message, index)))" class="mt-5">
                      <div class="flex flex-col gap-4 items-stretch">
                        ...
                        <Card class="w-full" :pt="{ root: { style: { background: 'var(--p-surface-800)', border: '1px solid var(--p-surface-600)', borderRadius: '8px', height: 'calc(13rem - 50px)', display: 'flex', flexDirection: 'column' } }, body: { style: { flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' } } }">
                          ...
                          <template #content>
                            <div class="grid grid-cols-4 w-full h-full items-center justify-items-center" style="column-gap: 30px; row-gap: 0px;">
                              <Button
                                v-for="n in 8"
                                :key="n"
                                text
                                ...
                                :aria-label="n === 2 ? 'Connect Salesforce' : n === 3 ? 'Connect Zoho' : n === 4 ? 'Connect Follow Up Boss' : n === 5 ? 'Connect Pipedrive' : 'Connect HubSpot'"
                                @click="n === 2 ? openCrmModal('Salesforce') : n === 3 ? openCrmModal('Zoho') : n === 4 ? openCrmModal('Follow Up Boss') : n === 5 ? openCrmModal('Pipedrive') : openCrmModal('HubSpot')"
                              >
                                <img
                                  v-if="n === 2"
