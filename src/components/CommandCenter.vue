              ...
              <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 items-stretch">
                  <div class="h-full flex flex-col">
                    <div class="flex flex-wrap gap-2 items-stretch h-full">
                      <div v-for="k in selected?.kpis || []" :key="k.label" :class="(selected?.kpis?.length || 0) <= 1 ? 'w-full h-full flex flex-col' : 'w-1/2 flex flex-col'">
                ...
                <div class="space-y-4">
                  ...
                  <div class="bg-gray-800/70 border border-gray-700 rounded-lg p-[22px]">
...
const statusCounts = computed(() => ({
  green: modules.value.filter(m => m.status === 'green').length,
  yellow: modules.value.filter(m => m.status === 'yellow').length,
  red: modules.value.filter(m => m.status === 'red').length
}))
...
function syncAssistantHeight() {
  ...
  const reduced = h > 20 ? h - 20 : h
  if (assistantPaneRef.value) assistantPaneRef.value.style.height = h ? `${reduced}px` : ''
}
